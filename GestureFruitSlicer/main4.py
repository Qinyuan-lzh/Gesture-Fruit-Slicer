...
trailPoints = []
trailLength = 15
spawnChanceFruit = 0.05
spawnChanceBomb = 0.005  # <<< 在这里调整炸弹几率
score = 0
freezeTime = 0.0
freezeDuration = 3.0
...
while True:
    ...
    # --- ⬇️⬇️⬇️ 新增：竖中指重置游戏 ⬇️⬇️⬇️ ---
    if hands and gameState != "MENU":  # 在菜单界面时不需要这个手势
        # 无论处于什么状态，都检查这个手势
        fingers = detector.fingersUp(hands[0])

        # 检查是否为竖中指: [拇指, 食指, 中指, 无名指, 小指]
        if fingers == [0, 0, 1, 0, 0]:
            print("检测到重置手势 (竖中指)，返回菜单...")
            gameState = "MENU"  # 重置游戏状态到菜单
    # --- ⬆️⬆️⬆️ --------------------------- ---
